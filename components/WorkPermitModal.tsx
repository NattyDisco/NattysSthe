...
const WorkPermitModal: React.FC<WorkPermitModalProps> = ({ isOpen, onClose, permit }) => {
  const { employees, addWorkPermit, updateWorkPermit } = useAppContext();
  const { t } = useI18n();
  
  const [isUploading, setIsUploading] = useState(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);

  const initialState: Omit<WorkPermit, 'id'> = {
    employeeId: '',
    permitNumber: '',
  ...
  useEffect(() => {
    if (permit) {
      setFormData({ ...initialState, ...permit });
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (permit) {
            await updateWorkPermit(permit.id, finalData);
        } else {
            await addWorkPermit(finalData);
        }
        onClose();
    } catch (error: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 shadow-2xl w-full max-w-lg border border-white/20" onClick={e => e.stopPropagation()}>
        <h2 className="text-3xl font-black mb-6 uppercase tracking-tighter">{permit ? 'Update Work Permit' : 'New Work Permit'}</h2>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest">Assign to Employee</label>
          ...
          <div>
              <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest">Permit Number</label>
              <input type="text" name="permitNumber" value={formData.permitNumber} onChange={handleChange} required placeholder="e.g. LSO-1234567" className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-2xl font-bold outline-none focus:border-indigo-500" />
          </div>
          ...
          <div>
              <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest">Digital Copy (Cloud Storage)</label>
              <div className="relative group">
                ...
                <div className="p-6 bg-slate-50 dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[2rem] text-center group-hover:border-indigo-500 transition-all">
                    ...
                    <p className="text-xs font-bold text-slate-500">{selectedFile ? selectedFile.name : (formData.fileName || 'Select Permit File')}</p>
                </div>
              </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" disabled={isUploading} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-indigo-500/30 hover:bg-indigo-700 transition-all active:scale-95">
                {isUploading ? "Uploading to Cloud..." : "Save Record"}
            </button>
          </div>
        </form>
      </div>
